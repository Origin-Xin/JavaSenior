 ...
 *          |----LinkedHashMap:保证在遍历map元素时，可以按照天际的顺序实现遍历。
 ...
 *          |----properties:常用俩处理配置文件。key和value都是String类型
 *
 ...
 *              如果key1的哈希值与已经存在的数据的哈希值相同，此时key1-value1添加成功。----情况2
 ...
 *
 *      在不断的添加过程中，会涉及到扩容问题，默认的扩容方式：扩容为原来的容量的2倍，并将原有的数据复制过来
 *
 ...
 *          当前的某一个索引位置上的元素以链表形式存在的数据个数 >8 且当数组长度 >64时
 ...
 *  元素查询的操作：
 *  Object get(Object key)：获取指定key对应   的value
